/**
 * Copyright (c) 2026 OpenBKN
 * SPDX-License-Identifier: LicenseRef-OpenBKN
 * Licensed under the OpenBKN License, a modified Apache 2.0 with Additional
 * Conditions. See LICENSE for the full text.
 */

export const bknTraceZhCN = {
  bknTrace: {
    businessProvenance: {
      description: "从业务会话、交互轮次和 OpenBKN 调用出发，解释业务结论及其数据、逻辑与行动依据。",
      title: "业务溯源分析",
    },
    traceAnalysis: {
      description: "按业务上下文或技术标识分析跨模块调用链。",
      title: "Trace 分析",
    },
    logs: {
      categoryPlaceholder: "日志类别",
      columns: { category: "类别", event: "事件摘要", service: "服务", severity: "级别", time: "发生时间", trace: "Trace" },
      description: "检索当前授权范围内的平台、业务、模型和审计日志，并下钻关联 Trace。",
      emptyAssociated: "所选时间范围内没有匹配当前 Trace 或 Request 的日志事件。",
      emptyRange: "所选时间范围内没有可见日志事件。",
      partialWarning: "部分日志来源暂不可用，当前结果可能不完整。",
      searchPlaceholder: "搜索安全摘要或事件",
      servicePlaceholder: "服务来源",
      timeRange: "日志时间范围",
      detail: {
        category: "日志类别", logId: "日志 ID", openBusinessProvenance: "打开关联业务溯源", openTrace: "打开关联 Trace", outcome: "执行结果",
        projectedFields: "有 {{count}} 个字段未进入当前受控投影。", projection: "字段投影策略",
        requestId: "Request ID", service: "服务", source: "来源", time: "发生时间", title: "日志详情", traceId: "Trace ID",
      },
      title: "日志检索",
    },
    settings: {
      columns: { category: "类别", collection: "采集方式", coverage: "覆盖模块", policyKind: "策略类型", retention: "保留时间", revision: "策略版本", source: "日志来源", status: "状态" },
      days: "天",
      description: "查看日志来源覆盖、采集状态与当前生效的存储保留策略。",
      policies: "保留与存储策略",
      readOnlyNotice: "日志保留策略为只读；Trace 与证据链变更由受管发布控制器执行并自动回滚失败发布。",
		traceEvidence: "Trace 与证据链",
		traceEvidenceDisabled: "已关闭",
		traceEvidenceEnabled: "已开启",
		traceEvidenceOperation: "发布中：{{phase}}",
		traceEvidenceFailed: "发布状态：{{phase}}（期望：{{desired}}，生效：{{effective}}）",
		confirmEnableTitle: "确认开启 Trace 与证据链？",
		confirmDisableTitle: "确认关闭 Trace 与证据链？",
		confirmReleaseImpact: "此操作会对受管服务执行滚动发布；发布期间新旧实例可能短暂并存。",
		traceEvidenceRevision: "配置版本 {{revision}}",
      sources: "来源覆盖",
      title: "可观测性设置",
    },
    actions: {
      back: "返回运行列表",
      loadMore: "加载更多",
      query: "查询",
      refresh: "刷新",
      viewLogs: "查看关联日志",
    },
    description: "按 trace id 或 request id 查看运行链路、证据链、业务语义链和快照预览。",
    empty: "输入 trace id 或 request id 后查询",
    errors: {
      accessDenied: "当前账号没有可用的溯源分析视图。",
      accessProfileFailed: "无法获取当前溯源分析权限，请刷新后重试。",
      missingScope: "请输入 trace id 或 request id。",
      queryFailed: "查询失败。",
      traceEvidenceUpdateFailed: "发布请求未被接受，已重新加载服务端最新状态；请确认后重试。",
    },
    emptyStates: {
      businessNodes: "未返回业务语义节点",
      claims: "未返回结论明细",
      evidenceRefs: "未返回证据引用明细",
      node: "选择节点查看明细",
      relations: "未形成可见的业务关系",
      runs: "暂无可观测业务运行",
      stage: "该阶段暂无可见事实",
      artifacts: "暂无可见的查询、数据、逻辑或行动制品",
    },
    complete: "证据链完整",
    completeness: {
      complete: "完整",
      contentUnavailable: "业务内容不可用",
      partial: "部分完整",
    },
    incompleteBusiness: "部分业务依据暂不可追溯",
	unnamedAgent: "未命名 Agent",
	conclusionAtInteraction: "本次 OpenBKN 调用记录执行事实，最终业务结论请在所属交互轮次查看。",
	durationUnits: { hour: "小时", millisecond: "毫秒", minute: "分钟", second: "秒" },
    edges: {
      caused_by: "直接原因",
      observed: "观测到",
      recommends: "建议",
      supports: "支持结论",
      transitions_to: "状态流转",
      uses_business_ref: "使用业务语义",
    },
    fields: {
      basis: "依据摘要",
      agentOrApp: "Agent / 应用",
	  agentName: "Agent",
	  applicationPrincipalId: "应用凭据 ID",
	  effectiveSubjectId: "实际账号",
	  inputSummary: "查询对象 / 输入摘要",
      businessDomain: "业务域",
      businessPath: "业务路径",
      claimId: "结论 ID",
      complianceStatus: "合规状态",
      conversationId: "会话 ID",
      controlledSummary: "受控摘要",
      duration: "耗时",
      evidenceCompleteness: "证据完整性",
      identifier: "标识",
      interactionId: "交互轮次 ID",
      interactionCount: "交互轮次",
      kind: "类型",
      mode: "模式",
      object: "对象",
      observed: "已观测",
      operationId: "操作 ID",
      operation: "业务操作",
	  operationResult: "调用结果",
      question: "用户问题",
      requestId: "Request ID",
      requestCount: "本轮调用数",
      result: "业务结果",
      retentionPolicy: "保留策略",
      resolutionStatus: "解析状态",
      service: "服务",
      snapshotId: "快照 ID",
      sourceVersion: "来源版本",
      span: "Span",
      startedAt: "开始时间",
      status: "状态",
      technicalDetails: "展开技术信息",
      traceId: "Trace ID",
      type: "类型",
      version: "版本",
      visibility: "可见性",
    },
    metrics: {
      businessEdges: "业务边",
      businessNodes: "业务节点",
      businessRefs: "业务引用",
      claims: "结论",
      evidenceRefs: "证据引用",
      spans: "Span",
    },
    partial: "当前结果不完整",
    placeholders: {
      agentOrApp: "Agent / 应用",
      businessDomain: "业务域",
      evidenceCompleteness: "证据完整性",
      keyword: "搜索问题、结果、业务对象或 ID",
      knowledgeNetwork: "知识网络",
      requestId: "request id",
      status: "运行状态",
      timeFrom: "开始时间",
      timeTo: "结束时间",
      traceId: "trace id",
    },
    scope: {
      request: "Request",
      trace: "Trace",
    },
    sections: {
      businessDetails: "业务语义节点明细",
      businessGraph: "业务语义链",
      artifacts: "数据、逻辑与行动依据",
      claimDetails: "结论明细",
      completeness: "完整性",
      evidenceDetails: "证据引用明细",
      evidenceChain: "证据链",
      nodeDetails: "节点明细",
      relations: "业务关系",
	  requestDetail: "OpenBKN 调用详情",
      interactionRequests: "本轮 OpenBKN 调用",
      snapshot: "快照预览",
      traceGraph: "调用链",
      visibility: "可见性",
    },
    stages: {
      action: "行动与结果",
      claim: "业务结论",
      evidence: "业务证据",
      execution: "执行过程",
      intent: "交互意图",
    },
    tabs: {
      advanced: "高级查询",
      business: "业务解释",
      diagnostics: "技术诊断",
      governance: "治理与快照",
      runs: "业务运行",
    },
    runsDescription: "从用户问题和业务结果出发，查看分析、数据、逻辑、行动与技术调用依据。",
	operationResults: {
	  completed: "调用完成，关联 {{count}} 项业务依据",
	  dataCount: "返回 {{count}} 条业务数据",
	  error: "调用失败",
	  noData: "未返回符合条件的业务数据",
	  running: "调用中",
	  unknown: "状态未知",
	},
	operations: {
	  calculateLogic: "计算业务逻辑或指标",
	  describeResource: "读取数据资源结构",
	  findSkills: "查找可用业务技能",
	  getActionInfo: "读取可执行行动",
	  getKnowledgeNetworkDetail: "读取知识网络详情",
	  listKnowledgeNetworks: "查找业务知识网络",
	  listResources: "查找数据资源",
	  queryObject: "查询业务对象",
	  querySubgraph: "沿业务关系查询",
	  runSql: "查询业务数据",
	  searchSchema: "检索知识网络结构",
	},
	 evidence: {
	  assembling: "证据组装中",
	  complete: "完整可溯源",
	  content_unavailable: "内容暂不可用",
	  failed: "证据生成失败",
	  not_applicable: "无需业务证据",
	  partial: "部分可溯源",
	 },
    status: {
	  abandoned: "交互已中断",
	  active: "持续进行中",
	  canceled: "交互已取消",
	  closed: "会话已结束",
      completed: "已完成",
      error: "失败",
	  expired: "会话已过期",
	  failed: "交互失败",
	  handed_off: "已转人工",
      running: "运行中",
	  unknown: "状态未知",
    },
    title: "BKN Trace",
    traceGraphRequestOnly: "request id 查询不返回调用链",
    visibility: {
      authorized: "已授权",
      hidden: "已隐藏",
      redacted: "已脱敏",
      unauthorized: "未授权",
      unresolved: "未解析",
    },
    views: {
      conversations: "业务会话",
      interactions: "交互轮次",
      requests: "OpenBKN 调用",
    },
  },
} as const;
